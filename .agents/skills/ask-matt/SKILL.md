...
- **`/implement`** — the standard closed loop (plan → baseline → implement → verify gate → review → log → memory).
...
No issue tracker setup required — this repo defaults to the local-markdown tracker: one file per issue under `.scratch/<feature>/issues/`. Referenced-but-optional skills (`/triage`, `/diagnosing-bugs`, `/domain-modeling`, `/research`, `/teach`, `/compact`) are part of the wider Matt Pocock skill ecosystem; add them here as needed.
